...
exports.getUsersToCallNow = getUsersToCallNow;
...
async function initiateCall(params) {
    ...
    try {
        ...
        const callProvider = (0, factory_1.getCallProvider)();
        ...
        console.log(`[CallService] Call initiated successfully via ${callProvider.name}: ${call.id} -> ${providerResponse.vendorCallId}`);
        return {
            success: true,
            callId: call.id,
 ...
 */
async function getUsersToCallNow() {
    try {
        const supabase = getServiceClient();
        ...
        for (const user of users) {
            ...
            const userTimezone = user.local_tz || config_1.config.defaultTimezone;
            const userLocalTime = new Date(now.toLocaleString('en-US', { timeZone: userTimezone }));
            const currentHour = userLocalTime.getHours();
            const currentMinute = userLocalTime.getMinutes();
            // Parse user's call_time (format: "HH:MM:SS")
            const userCallTime = user.call_time || config_1.config.defaultCallTime;
            ...
            if (currentTotalMinutes >= callTotalMinutes &&
                currentTotalMinutes < callTotalMinutes + config_1.config.callTimeWindowMinutes) {
                usersToCall.push({
                    id: user.id,
                    phone: user.phone,
                    name: user.name,
