...
ALTER TABLE public.users
  ...
  ADD COLUMN IF NOT EXISTS call_time TIME DEFAULT '20:30:00';

-- Add comments
COMMENT ON COLUMN public.users.call_enabled IS 'Whether user has enabled automated daily calls (default: true)';
COMMENT ON COLUMN public.users.call_time IS 'Preferred call time in user''s local timezone (default: 8:30 PM)';
